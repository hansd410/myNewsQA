...
#        contextSenList = nltk.sent_tokenize(story_text.decode('utf-8',errors='ignore'))
#        contextSenList = nltk.sent_tokenize(story_text.decode('utf-8'))
        try:
            contextSenList = nltk.sent_tokenize(story_text)
        except:
            continue
        ...
        for j in range(len(contextSenList)):
            try:
                contextSenList[j].decode('ascii')
            except:
                continue
            if(j == 0):
                preIndex[1] += len(contextSenList[j])
            if(j == 1):
                preIndex[0] += len(contextSenList[j-1])
                preIndex[1] += len(contextSenList[j])+1
            else:
                preIndex[0] += len(contextSenList[j-1])+1
                preIndex[1] += len(contextSenList[j])+1
            ...
            threeContextSen = ""
            if(len(contextSenList) == 1):
                threeContextSen = contextSenList[j]
            elif(len(contextSenList) == 2):
                if(j==0):
                    threeContextSen = contextSenList[j]+" "+contextSenList[j+1]
                else:
                    threeContextSen = contextSenList[j-1]+" "+contextSenList[j]
            else:
                if(j==0):
                    threeContextSen = contextSenList[j]+" "+contextSenList[j+1]
                else:
                    if(j==len(contextSenList)-1):
                        threeContextSen = contextSenList[j-1]+" "+contextSenList[j]
                    else:
                        threeContextSen = contextSenList[j-1]+" "+contextSenList[j]+" "+contextSenList[j+1]
            ...
            for k in beginListIndex:
                answer = answerList[k]
                try:
                    beginIndex = threeContextSen.find(answer)
                except:                  
                    continue
                if(beginIndex > len(threeContextSen) or beginIndex<0):
                    continue
                try:
        ...
        ##### print to JSON #####            
                jsonParagraph=[{"context":threeContextSen,"qas":[{"answers":jsonAnswers,"question":questions,"id":tempStoryId}]}]
